...
def B(): #TODO b) Use the Mazzer tool to unlock the Mazzer Scale.
    ...
    UR5.MoveL([117.580000, -93.770000, 117.650000, -113.010000, -89.130000, -220]) #intermeidate point to avoid the mazzer
    ...
    UR5.MoveL(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -24, pos_z=-9,theta_x=-120,off_theta_z=150), blocking=True)
    # print("down and across to unlock the lock")
    UR5.MoveL(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -24, pos_z=50,theta_x=-120, off_theta_z=150), blocking=True)
    # print("leave lock")
...
def C(): #TODO c) Use the Mazzer tool to turn the Mazzer on, wait 15s, and turn the Mazzer off.
    ...
    UR5.MoveL(tf.pose(points_df, id.Mazzer_On_Button, tool=id.Mazzer_Tip_Tool, pos_y=10,theta_y=-110, off_z= 0), blocking=True) #go to on button
    UR5.MoveL(tf.pose(points_df, id.Mazzer_On_Button, tool=id.Mazzer_Tip_Tool, pos_y=10,theta_y=-110, off_z= -10), blocking=True) #push on button
    the_time = time.time()
...
#spin_start_pose = robomath.TxyzRxyz_2_Pose([0,0,-2,0,0,0]) * tf.pose(points_df, id.Rancillio_Gasket, tool=id.Rancillio_Basket_Tool, pos_z=0, theta_y=-90, off_theta_x=45)
...
def L(): #TODO l) Remove the Rancilio tool from the PUQ fixture, and insert it into the Rancilio group head.
    ...
    UR5.MoveJ([-1.271754, -105.475013, 144.718062, -36.002757, 24.039748, -222.739798])
    ...
    final_pose =  UR5.Pose() 
    UR5.MoveL(robomath.TxyzRxyz_2_Pose([0,0,-2,0,0,0]) * final_pose * robomath.TxyzRxyz_2_Pose([-1,0,0,0,0,0]))
...
def M():# #TODO m) Use the Mazzer tool to operate the cup dispenser.
    ...
    UR5.MoveJ([19.490000, -102.380000, 102.600000, -89.470000, -93.710000, -219.73])
    tls.mazzer_tool_detach_l_ati() 

def N():
    run_visual_program(RDK, 'Show_Cup_Dispenser_Cup')
    tls.cup_tool_attach_l_ati()
    UR5.MoveJ([5.460000, -98.857406, 142.387657, -114.440000, 100.836905, -228.360000])
    UR5.MoveL([10.671531, -98.857406, 142.387657, -43.543240, 100.836905, -219.639732])
    ...
    UR5.MoveJ([19.140000, -96.590000, 129.140000, -33.570000, 98.280000, -219.620000])
    # Go to the cup approach postison
    UR5.MoveL(tf.pose(points_df, id.Rancillio_Scale_Centre, tool=id.Cup_Closed_Tool, pos_z=77, theta_y= 90, theta_x=90, theta_z=90), blocking=True)
    tls.cup_tool_open_ur5()
    run_visual_program(RDK, 'Show_Rancilio_Scale_Cup', blocking=True)
...
def Q(): #Use the Mazzer tool to lock the Rancilio Scale.
    ...
    UR5.MoveJ(tf.pose(points_df, id.Rancillio_Scale, tool=id.Mazzer_Tip_Tool, pos_x=31.5, pos_y=21.53, pos_z=0, theta_x = 125), blocking=True)
    # Flipping Ranccilio Scale Switch
    UR5.MoveJ(tf.pose(points_df, id.Rancillio_Scale, tool=id.Mazzer_Tip_Tool, pos_x=31.5+25, pos_y=+25, pos_z=-34, theta_x = 135), blocking=True)
...
def V(): # Return the Rancilio tool to the tool stand.
    UR5.MoveJ(tf.pose(points_df, 61, tool=id.Rancillio_Basket_Tool, pos_z = 100, pos_y=100, theta_z=-90, theta_x=90), blocking=True)
...
def R(): #Use the cup tool to carefully pick up the cup of coffee and place it in the customer zone.
    ...
    UR5.MoveJ([-9.352821, -101.478139, 135.041891, -34.930757, 20.766093, -218.572880])
...
UR5.MoveJ([180.000000, -90.000000, 90.000000, -90.000000, -90.000000, -90.000000])
