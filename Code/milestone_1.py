...
UR5.MoveJ(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -15, pos_z=50,theta_y=180, theta_z=180), blocking=True)
print("move to above the lock")
UR5.MoveL(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -15, pos_z=-5,theta_y=180, theta_z=180), blocking=True)
print("move to unlock the lock")
UR5.MoveL(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -20, pos_z=-7,theta_y=180, theta_z=180), blocking=True)
print("move to unlock the lock")
UR5.MoveJ(tf.pose(points_df, id.Mazzer_Scale_Lock, tool=id.Mazzer_Tip_Tool, pos_x= -15, pos_z=50,theta_y=180, theta_z=180), blocking=True)
print("move to above the lock")
...
UR5.MoveJ([117.580000, -93.770000, 117.650000, -113.010000, -89.130000, -204.190000]) #intermeidate point to avoid the mazzer
time.sleep(1)
UR5.MoveJ(tf.pose(points_df, id.Mazzer_On_Button, tool=id.Mazzer_Tip_Tool, pos_y=10,theta_y=-110, off_z= -10), blocking=True)
UR5.MoveJ(tf.pose(points_df, id.Mazzer_On_Button, tool=id.Mazzer_Tip_Tool, pos_y=10,theta_y=-110), blocking=True)
the_time = time.time()
UR5.MoveJ(tf.pose(points_df, id.Mazzer_On_Button, tool=id.Mazzer_Tip_Tool, pos_y=10,theta_y=-110, off_z= -10), blocking=True)
...
while (time.time() - the_time) < 1: # wait 15 secs
    ...
    pass # do nothing
UR5.MoveJ(tf.pose(points_df, id.Mazzer_Off_Button, tool=id.Mazzer_Tip_Tool, pos_y=15,theta_y=-130), blocking=True) #push off button
UR5.MoveJ(tf.pose(points_df, id.Mazzer_Off_Button, tool=id.Mazzer_Tip_Tool, pos_y=15,theta_y=-130, off_z=-5), blocking=True) #go to off button
